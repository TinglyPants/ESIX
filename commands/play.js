...
module.exports = {
    ...
    async execute(interaction){
        ...
        if (interaction.options.getSubcommand() === "query"){
            yt_info = await play.search(interaction.options.getString('query'), {
                limit: 1
            })
            yt_info = yt_info[0]
            ...
            yt_info = await play.video_info(interaction.options.getString('url'))
        ...
        if (player._state.status === 'idle'){
            ...
            responseEmbed.setTitle("Added to queue")
            .addFields({
                name: resource.songAuthor,
                value: resource.songTitle
            })
